...
describe('Container', () => {
  ...
  it('caches singleton providers', async () => {
    ...
    let counter = 0;
    container.registerFactory(TOKEN, () => ({ id: ++counter }), [], 'SINGLETON');
  ...
  it('creates new instance for transient providers', async () => {
    ...
    const TOKEN = createToken<{ id: number }>('test');
    let counter = 0;
    container.registerFactory(TOKEN, () => ({ id: ++counter }), [], 'TRANSIENT');
  ...
  it('throws on missing provider', async () => {
    ...
    await expect(container.resolve(TOKEN)).rejects.toThrow('No provider registered');
  });
