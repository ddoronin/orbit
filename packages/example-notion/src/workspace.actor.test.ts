...
async function freshWorkspace(ownerId = 'u-owner') {
  ...
  await handle.call<WorkspaceState>('workspace.create', { name: 'Acme', ownerId });
  return handle;
}
...
describe('WorkspaceActor', () => {
  ...
  it('workspace.create refuses a second init', async () => {
    ...
    await expect(handle.call('workspace.create', { name: 'X', ownerId: 'u2' }))
  ...
  it('workspace.invite adds a member with the given role', async () => {
    ...
    await handle.call('workspace.invite', {
      inviterId: 'u-owner', userId: 'u-bob', role: 'editor',
    });
  ...
  it('workspace.invite rejects non-owner/editor invitations', async () => {
    const handle = await freshWorkspace('u-owner');
    await handle.call('workspace.invite', {
      inviterId: 'u-owner', userId: 'u-vw', role: 'viewer',
    });
    await expect(handle.call('workspace.invite', {
      inviterId: 'u-vw', userId: 'u-x', role: 'editor',
  ...
  it('workspace.createPage adds page and tracks as root by default', async () => {
    ...
    const summary = await handle.call('workspace.createPage', {
      authorId: 'u-owner', pageId: 'p-1', title: 'Hello',
    });
    expect(summary).toMatchObject({ pageId: 'p-1', title: 'Hello', parentPageId: null });
  ...
  it('workspace.createPage records parent and does not duplicate in rootPageIds', async () => {
    const handle = await freshWorkspace();
    await handle.call('workspace.createPage', { authorId: 'u-owner', pageId: 'parent', title: 'P' });
    await handle.call('workspace.createPage', {
      authorId: 'u-owner', pageId: 'child', title: 'C', parentPageId: 'parent',
    });
  ...
  it('workspace.createPage requires owner/editor role', async () => {
    const handle = await freshWorkspace();
    await handle.call('workspace.invite', { inviterId: 'u-owner', userId: 'u-vw', role: 'viewer' });
    await expect(handle.call('workspace.createPage', {
      authorId: 'u-vw', pageId: 'p', title: 'X',
    })).rejects.toThrow(/lacks required role/);
  });

  it('workspace.updatePageSummary patches title/icon/updatedAt', async () => {
    const handle = await freshWorkspace();
    await handle.call('workspace.createPage', { authorId: 'u-owner', pageId: 'p', title: 'Old' });
    await handle.call('workspace.updatePageSummary', {
      pageId: 'p', title: 'New', icon: '📝', updatedAt: 12345,
    });
  ...
  it('workspace.listPages filters by parent and orders by updatedAt desc', async () => {
    const handle = await freshWorkspace();
    await handle.call('workspace.createPage', { authorId: 'u-owner', pageId: 'a', title: 'A' });
    await handle.call('workspace.createPage', { authorId: 'u-owner', pageId: 'b', title: 'B' });
    await handle.call('workspace.createPage', {
      authorId: 'u-owner', pageId: 'c', title: 'C', parentPageId: 'a',
    });
    ...
    const roots = await handle.call('workspace.listPages', { parentPageId: null });
    expect((roots as any[]).map((p) => p.pageId)).toEqual(['b', 'a']);
    const children = await handle.call('workspace.listPages', { parentPageId: 'a' });
