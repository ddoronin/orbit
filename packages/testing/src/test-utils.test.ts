...
import { createTestContainer, createTestActor, createTestApp } from './test-utils.js';
...
class CounterActor extends OrbitActor<{ n: number; swept: boolean }> {
  initialState() { return { n: 0, swept: false }; }
  @Handle('inc') async inc() { this.updateState((s) => { s.n++; }); return this.state.n; }
  @Handle('boom') async boom() { throw new Error('explode'); }
  @OnAlarm() async sweep() { this.updateState((s) => { s.swept = true; }); }
}
...
describe('createTestApp', () => {
  ...
  it('serializes JSON bodies and sets Content-Type', async () => {
    const router = new Router().post('/echo', async (ctx) => ctx.json(await ctx.body()));
    const app = createTestApp(router);
    const res = await app.request('/echo', { method: 'POST', body: { greet: 'hi' } });
